...
SoftwareSerial gps_serial(NEO_6M_GPS_SENSOR_RX, NEO_6M_GPS_SENSOR_TX); // The serial connection to the GPS device
    ...
    for (uint8_t trials = 0; trials < max_trials; trials++)
    {
        if (gps_serial.available() > 0)
        {
            ...
            if (gps.location.isUpdated())
            {
                ...
                gps_serial.end();
                return ("X:" + (String)latitude + ",Y:" + (String)longitude);
            }
        }
    }
